...
file = r"/Users/mariawarter/Box Sync/PhD/PhD/Riparian Model/Data/ag_temp_val.csv"
ag_temp= pd.read_csv(file,dayfirst=True,parse_dates=True, index_col=[0])
...
#ag_temp = ag_temp.loc[:'2019-10-31']
...
df_sos_gr = pd.concat([sos_savi_gr['SOS-SAVI'],rain['94D']],axis=1) #check duplicate values 
...
#
rain=pd.DataFrame(ag_rain['94D']).loc[ag_rain['DOWY'] == 173].round(0)

sob_gr = pd.DataFrame(0.001*rain +0.063 )#SHRUB

sob_gr.columns=['SOB-SAVI']


sob_shr = pd.DataFrame(0.0007*rain +0.08)
...
min_savi_gr = min_savi_gr.set_index(pd.DatetimeIndex(temp_min_doy.slope))
...
artifical_gr=pd.concat([distr_max_gr.iloc[:,0:100], distr_eos_gr.iloc[:,0:100], 
                        distr_sob_gr.iloc[:,0:100],distr_sos_gr.iloc[:,0:100],
                        distr_som_gr.iloc[:,0:100],min_savi_gr.iloc[:,0:100]])
#
artifical_gr=artifical_gr.sort_index(ascending=True)
artifical_gr=artifical_gr.iloc[:,0:100].clip(lower=0.05)
artifical_gr['Date'] =artifical_gr.index
artifical_gr['WY'] = artifical_gr.apply(lambda x: assign_wy(x), axis=1)
artifical_gr['DOWY'] = artifical_gr.index.to_series().apply(day_of_water_year)
artifical_gr['median'] = artifical_gr.iloc[:,0:100].median(axis=1)
...
#%%
artifical_shr=pd.concat([distr_max_shr.iloc[:,0:100],distr_eos_shr.iloc[:,0:100],
                         distr_sob_shr.iloc[:,0:100], distr_sos_shr.iloc[:,0:100], 
                         distr_som_shr.iloc[:,0:100],min_savi_shr.iloc[:,0:100]])
#
artifical_shr=artifical_shr.sort_index(ascending=True)
artifical_shr=artifical_shr.iloc[:,0:100].clip(lower=0.05)
artifical_shr['Date'] =artifical_shr.index
artifical_shr['WY'] = artifical_shr.apply(lambda x: assign_wy(x), axis=1)
artifical_shr['DOWY'] = artifical_shr.index.to_series().apply(day_of_water_year)
artifical_shr['median'] = artifical_shr.iloc[:,0:100].median(axis=1)
#artifical_shr=artifical_shr.loc[:'2019-10-31']
#artifical_shr = artifical_shr.loc[(artifical_shr.index < '2011-11-01') | (artifical_shr.index > '2013-11-01')]

#%%

ag_art_gr = pd.pivot_table(artifical_gr, index=artifical_gr['DOWY'], columns=artifical_gr['WY'],values='median')

ag_art_shr = pd.pivot_table(artifical_shr, index=artifical_shr['DOWY'], columns=artifical_shr['WY'],values='median')
...
art_gr = pd.concat([mod0,mod1, mod2,mod4,mod5,mod6,mod8,mod9,mod10],axis=1)
art_gr.columns=['1996','1998','2002','2006','2008','2010','2014','2016','2018']

#%%
art_shr = pd.concat([mod0,mod1, mod2,mod4,mod5,mod6, mod8,mod9,mod10],axis=1)
art_shr.columns=['1996','1998','2002','2006','2008','2010','2014','2016','2018']

#%% Plot seasonal mean actual SAVI values 
af_gr=art_gr.mean(axis=1)
af_gr_std=art_gr.std(axis=1)


af_shr=art_shr.mean(axis=1)
af_shr_std=art_shr.std(axis=1)
#%%
his_gr=ag_gr.mean(axis=1)
his_gr_std=ag_gr.std(axis=1)


his_shr=ag_shr.mean(axis=1)
his_shr_std=ag_shr.std(axis=1)

#%%

savi_grass=art_gr
savi_grass.drop(savi_grass.tail(1).index,inplace=True)
savi_grass= savi_grass.melt()
savi_grass = savi_grass.set_index(pd.DatetimeIndex(ag_rain['Date']))  
savi_grass.columns=['Year','SAVI']

savi_shrub=art_shr
savi_shrub.drop(savi_shrub.tail(1).index,inplace=True)
savi_shrub= savi_shrub.melt()
savi_shrub = savi_shrub.set_index(pd.DatetimeIndex(ag_rain['Date']))  
savi_shrub.columns=['Year','SAVI']
#%%
#savi_shrub.to_csv('savi_shr_artifical.csv',sep=',')
#savi_grass.to_csv('savi_gr_artifical.csv',sep=',')

#%%
gr=ag_gr.melt()
shr=ag_shr.melt()

ab=np.asarray(savi_grass['SAVI'])
std_savi_grass=savi_grass['SAVI'].std()
ac=np.asarray(savi_shrub['SAVI'])
std_savi_shrub=savi_shrub['SAVI'].std()

#%% TIMESERIES OF OBSERVED AND SYNTHETIC SAVI 
plt.subplot(211)
plt.plot(ab,color='red', label='synthetic')
plt.fill_between(gr.index,ab-std_savi_grass, ab+std_savi_grass,  color='red',alpha=0.1) #Modelled std
plt.plot(np.asarray(gr['value']),color='grey',label='observed')
plt.grid(alpha=0.2)
plt.ylim(0,0.31)
plt.legend(frameon=False)
plt.subplot(212)
plt.plot(ac,color='red',label='synthetic')
plt.fill_between(shr.index,ac-std_savi_shrub, ac+std_savi_shrub,  color='red',alpha=0.1) #Modelled std
plt.plot(np.asarray(shr['value']),color='grey',label='observed')
plt.grid(alpha=0.2)
plt.ylim(0,0.31)

#%% MEAN SAVI VALUES PLOT
plt.subplot(121)
plt.plot(af_gr,color='green',label='Modelled SAVI') #Modelled 
plt.fill_between(af_gr.index,af_gr-af_gr_std, af_gr+af_gr_std,  color='green',alpha=0.3, edgecolor='green') #Modelled std
plt.plot(his_gr,color='black',label='Historic SAVI') #historic 
plt.fill_between(his_gr.index,his_gr-his_gr_std, his_gr+his_gr_std,  color='grey',alpha=0.3, edgecolor='grey') #historic std
plt.grid(alpha=0.2)
plt.ylim(0.04,0.27)
plt.ylabel('SAVI')
plt.legend(frameon=False)
plt.subplot(122)
plt.plot(af_shr,color='orange',label='Modelled SAVI')
plt.fill_between(af_shr.index,af_shr-af_shr_std, af_shr+af_shr_std,  color='orange',alpha=0.3, edgecolor='orange')
plt.plot(his_shr,color='black',label='Historic SAVI')
plt.fill_between(his_shr.index,his_shr-his_shr_std, his_shr+his_shr_std,  color='grey',alpha=0.3, edgecolor='grey') #historic std
plt.ylim(0.04,0.27)
plt.ylabel('SAVI')
plt.legend(frameon=False)
plt.grid(alpha=0.2)
...
tst1=pd.DataFrame(ag_shr['median'][ag_shr.index.isin(artifical_shr.index)])
tst2=artifical_shr.iloc[:,0:100].mean(axis=1)

print(stats.ks_2samp(tst1['median'],tst2)) #p=0.21, s=0.19
#%%

tst3=pd.DataFrame(ag_gr['median'][ag_gr.index.isin(artifical_gr.index)])
tst4=artifical_gr.iloc[:,0:100].mean(axis=1)

print(stats.ks_2samp(tst3['median'],tst4)) #p=0.12, s=0.21
#%%
plt.subplot(211)
sns.kdeplot(af_gr,color='green',label='Modelled')
sns.kdeplot(his_gr,color='green',linestyle='--',label='Observed')
plt.ylim(0,20)
plt.xlim(0,0.25)
plt.grid(alpha=0.2)
plt.legend(frameon=False)
plt.subplot(212)
sns.kdeplot(af_shr,color='orange')
sns.kdeplot(his_shr,color='orange',linestyle='--')
plt.ylim(0,20)
plt.grid(alpha=0.2)
plt.xlim(0,0.25)
plt.xlabel('SAVI')
...
y=df_max_gr['MAX-SAVI']

print(stats.kruskal(x,y)) #p=0.61
#%%###SHRUB
x= ag_shr.max(axis=0)

y=df_max_shr['MAX-SAVI']

print(stats.kruskal(x,y)) #p=1.0

#%%
#MINIMUM SAVI
x=x= ag_gr.min(axis=0)

y=min_savi_gr.iloc[:,0:99].mean(axis=1)

print(stats.kruskal(x,y)) #p=0.82

#%%## SHRUB
x= ag_shr.min(axis=0)

y=min_savi_shr.iloc[:,0:99].mean(axis=1)

print(stats.kruskal(x,y)) #p=0.52

#%%
#AT START OF BROWNING =SPRING PEAK 
x= ag_gr.iloc[176]

y= df_sob_gr['SOB-SAVI']
...
y= df_sob_shr['SOB-SAVI']

print(stats.kruskal(x,y)) #p=0.12

#%% START OF MONSOON 

x= ag_gr['median'][ag_gr.index.isin(distr_som_gr.index)]

y= df_som_gr['SOM-SAVI']

print(stats.kruskal(x,y) )#p=0.93
#%%%#############SHRUB
x= ag_shr['median'][ag_shr.index.isin(distr_som_shr.index)]

y= df_som_shr['SOM-SAVI']

print(stats.kruskal(x,y)) #p=0.80

#%% End of season 
#SHRUB
x= ag_shr.iloc[365]

y= df_sos_shr['SOS-SAVI']

print(stats.kruskal(x,y)) #p=0.45
#%%
x= ag_gr.iloc[365]

y= df_sos_gr['SOS-SAVI']

print(stats.kruskal(x,y)) #p=0.50
