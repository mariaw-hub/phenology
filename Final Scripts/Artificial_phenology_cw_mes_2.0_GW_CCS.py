...
@author: mariawarter
"""
Run the original GW model file first to get the simulated reference. Then run this file to the the CCS output. 
Check df_max for weird values(doubles) and that they are uneven values (ci only has uneven= )
Check artifical_ccs for weird median/minimum values 
...
ax_cw=ax_cw[ax_cw['WY'].isin([2001,2002,2003,2006,2008,2010,2011,2014,2015,2016,2017])]
...
ax_mes=ax_mes[ax_mes['WY'].isin([2001,2002,2003,2006,2008,2010,2011,2014,2015,2016,2017])]
...
ax_gw=ax_gw[ax_gw['WY'].isin([2002,2003,2006,2008,2010,2014,2015,2016,2017])]
ax_gw1=ax_gw['DTG']
#%%
ax_gw['DTGc'] = pd.DataFrame((ax_gw['DTG17']+0.5)).round(2)
ax_gw['DTGcn'] =pd.DataFrame((ax_gw['DTGc']- ax_gw['DTG17'].min())/(ax_gw['DTG17'].max()-ax_gw['DTG17'].min()))
ax_gw['Date'] =ax_gw.index
ax_gw['WY'] = ax_gw.apply(lambda x: assign_wy(x), axis=1)
ax_gw['DOWY'] = ax_gw.index.to_series().apply(day_of_water_year)
ax_gw2=ax_gw['DTGcn']
...
dc=pd.pivot_table(ax_temp, index=ax_temp['DOWY'], columns=ax_temp['WY'], values='96D')
...
ax_temp['WY'] = ax_temp.apply(lambda x: assign_wy(x), axis=1)
ax_temp['DOWY'] = ax_temp.index.to_series().apply(day_of_water_year)
ax_temp = ax_temp[ax_temp.DOWY != 366]

ax_temp=ax_temp[ax_temp['WY'].isin([2002,2003,2006,2008,2010,2014,2015,2016,2017])]
#%

dd=pd.pivot_table(ax_temp, index=ax_temp['DOWY'], columns=ax_temp['WY'], values='96D')
dd.mean(axis=1).plot()
dc.mean(axis=1).plot()
...
max_dtg = pd.DataFrame(ax_gw['DTGcn'][ax_gw.index.isin(max_doy.slope)]).round(2)
...
ci_max_cw['DTG'] = pd.Series(np.linspace(0, 2, 100)).round(2) #increase ci becasue DTG will be deeper under CCS
...
ci_max_mes['DTG'] = pd.Series(np.linspace(0,2,100)).round(2)
#ci_max_mes=ci_max_mes.drop_duplicates(subset=['DTG'])

#%%
#Calculate the max savi values 
max_savi_cw = pd.DataFrame(-0.356*max_dtg['DTGcn']+0.607) #Cottonwood - calcualte with relative difference not actual values
max_savi_cw.columns=['MAX-SAVI']
#max_savi_cw=max_savi_cw.round(1)

max_savi_mes = pd.DataFrame(-0.268*max_dtg['DTGcn']+0.558) #Mesquite 
...
df_max_cw = pd.concat([max_savi_cw['MAX-SAVI'],max_dtg['DTGcn']],axis=1) #36 and 26 change 
df_max_mes = pd.concat([max_savi_mes['MAX-SAVI'],max_dtg['DTGcn']],axis=1)
#if it doesnt work, check that WTD values are all rounded to 1 decimal point 
#%%
df_max_cw=df_max_cw.merge(ci_max_cw, left_on='DTGcn', right_on='DTG')[['MAX-SAVI', 'ci','DTG']]
df_max_mes=df_max_mes.merge(ci_max_mes, left_on='DTGcn', right_on='DTG')[['MAX-SAVI', 'ci','DTG']]
...
eos_savi = pd.DataFrame(np.random.normal(loc=0.19, scale=0.026, size=[9,100])) #COTTONWOOD
distr_eos_cw = eos_savi.set_index(pd.DatetimeIndex(temp_eos['Date']))

eos_savi = pd.DataFrame(np.random.normal(loc=0.19, scale=0.02, size=[9,100])) #MESQUITE
...
sos_savi = pd.DataFrame(np.random.normal(loc=0.19, scale=0.03, size=[9,100]))
...
artifical_cw_ccs=pd.concat([distr_max_cw,distr_eos_cw,distr_sos_cw,min_savi_cw.iloc[:,0:100]]).round(2)
...
art_cw_ccs = pd.concat([mod2,mod3,mod4,mod5,mod6,mod8,mod9,mod10,mod11],axis=1)
art_cw_ccs.columns=['2002','2003','2006','2008','2010','2014','2015','2016','2017']

#%%
art_mes_ccs = pd.concat([mod2,mod3,mod4,mod5,mod6,mod8,mod9,mod10,mod11],axis=1)
art_mes_ccs.columns=['2002','2003','2006','2008','2010','2014','2015','2016','2017']
...
savi_cw_ccs = savi_cw_ccs.set_index(pd.DatetimeIndex(ax_mes['Date']))  
...
plt.subplot(421)
plt.plot(np.asarray(ax_gw1),color='blue',label='Historic')
plt.plot(np.asarray(ax_gw2),color='red',label='CCS')
plt.grid(alpha=0.2)
...
#x3= ax_gw2[ax_gw2.index.map(lambda t: t.month in months)]

#x2=x2['SAVI'][x2.index.isin(x3.index)]
...
x1= pd.DataFrame(savi_mes['SAVI'].groupby(savi_mes.index.year).max()).round(2)
x2= pd.DataFrame(savi_mes_ccs['SAVI'].groupby(savi_mes_ccs.index.year).max()).round(2)
...
#%%
predictions = np.asarray(savi_mes_ccs['SAVI'])
targets=np.asarray(savi_mes['SAVI'])
